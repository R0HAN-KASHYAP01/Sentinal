...
class _InspectorProfileScreenState extends State<InspectorProfileScreen> {
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      ...
      setState(() {
        ...
        _loadError = profile == null ? 'Inspector profile not found.' : null;
      });
    } catch (_) {
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _detectLocation() async {
    ...
    try {
      ...
      if (permission == LocationPermission.denied) {
        _showError('Location permission denied. Please allow location access to continue.');
        return;
      }
      if (permission == LocationPermission.deniedForever) {
        _showError('Location permission is permanently denied. Please enable it from your device settings.');
        return;
      }
      ...
      _showError('Timed out while getting your location. Please try again.');
    } on LocationServiceDisabledException {
      _showError('Location services are turned off. Please enable GPS and try again.');
    } on PostgrestException {
      _showError('Could not save your location. Please try again.');
    } catch (_) {
      _showError('Something went wrong while detecting your location.');
    } finally {
  ...
  Widget _buildBody() {
    if (_loadingProfile) return const LoadingState(message: 'Loading your profile...');
      ...
      builder: (context, constraints) {
          ...
          child: ClipRRect(
            ...
            child: profile.hasLocation
                ...
                : const EmptyState(
                    ...
                    message: 'Detect your current location to see nearby institutes on the map.',
                  ),
          ),
        );
        ...
        if (isWide) {
          return Row(
            ...
            children: [
              ...
              Expanded(flex: 3, child: SizedBox(height: constraints.maxHeight, child: mapSection)),
            ],
          );
        }
        ...
        return SingleChildScrollView(
          child: Column(
            children: [
              infoColumn,
              SizedBox(height: 360, child: mapSection),
            ],
          ),
        );
      },
            ...
            Row(
              children: [
                const CircleAvatar(
                  ...
                  child: Icon(Icons.badge_outlined, color: Colors.white, size: 28),
                ),
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Current Location', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
            const SizedBox(height: 12),
            if (profile.hasLocation) ...[
              _InfoRow(
                icon: Icons.my_location,
                label: 'Coordinates',
                value: '${profile.latitude!.toStringAsFixed(5)}, ${profile.longitude!.toStringAsFixed(5)}',
              ),
              if (profile.locationUpdatedAt != null)
                _InfoRow(icon: Icons.update, label: 'Last updated', value: _formatDate(profile.locationUpdatedAt!)),
            ] else
              const Text('Your current location has not been detected yet.',
            ...
            ElevatedButton.icon(
              ...
              label: Text(_detectingLocation ? 'Detecting location...' : 'Detect My Current Location'),
            ),
          ],
        ),
      ),
